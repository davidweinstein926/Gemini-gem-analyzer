...
def detect_light_source_from_filename(filename):
    """
    Detect light source from filename based on 3rd character from right
    Examples: 52BC1.txt -> B (Halogen), 51LC1.txt -> L (Laser), 52UC1.txt -> U (UV)
    """
    try:
        # Remove file extension
        base_name = filename.replace('.txt', '').replace('.csv', '')
        
        print(f"🔍 Analyzing filename: '{filename}' -> base: '{base_name}'")
        
        # Check if filename has at least 3 characters
        if len(base_name) < 3:
            print(f"⚠️ Filename '{filename}' too short to detect light source")
            return None, "Unknown"
        
        # Get 3rd character from right (index -3)
        light_char = base_name[-3].upper()
        
        print(f"🔍 3rd character from right: '{light_char}'")
        print(f"    Full breakdown: {base_name} -> positions from right:")
        for i in range(min(5, len(base_name))):
            pos = -(i+1)
            char = base_name[pos]
            print(f"      Position {i+1}: '{char}' (index {pos})")
        ...
        if light_char in light_source_map:
            light_name, light_code = light_source_map[light_char]
            print(f"✅ Detected light source: {light_name} ({light_code}) from character '{light_char}'")
            return light_code, light_name
        else:
            print(f"⚠️ Unknown light source code '{light_char}' in filename '{filename}'")
            return None, "Unknown"
            
    except Exception as e:
        print(f"❌ Error detecting light source from '{filename}': {e}")
        return None, "Unknown"

def normalize_spectrum_by_light_source(df, light_source, filename):
    """
    Apply light-source-specific normalization:
    - B/Halogen: 650nm → 50000, then scale to 0-100
    - L/Laser: max → 50000, then scale to 0-100  
    - U/UV: 811nm → 15000, then scale to 0-100
    """
    try:
        wavelengths = df.iloc[:, 0].copy()
        intensities = df.iloc[:, 1].copy()
        
        print(f"\n🔧 APPLYING {light_source} NORMALIZATION:")
        print(f"   Filename: {filename}")
        
        if light_source == 'B':  # Halogen
            return normalize_halogen_650nm_to_50k(wavelengths, intensities)
        elif light_source == 'L':  # Laser  
            return normalize_laser_max_to_50k(wavelengths, intensities)
        elif light_source == 'U':  # UV
            return normalize_uv_811nm_to_15k(wavelengths, intensities)
        else:
            print(f"⚠️ Unknown light source '{light_source}' - using default max normalization")
            return normalize_default_max_to_50k(wavelengths, intensities)
            
    except Exception as e:
        print(f"❌ Normalization error: {e}")
        return df

def normalize_halogen_650nm_to_50k(wavelengths, intensities):
    """B/Halogen: Normalize 650nm to 50000, then scale to 0-100"""
    
    print(f"🔧 HALOGEN NORMALIZATION DEBUG:")
    print(f"   Input spectrum range: {np.min(intensities):.2f} - {np.max(intensities):.2f}")
    print(f"   Wavelength range: {np.min(wavelengths):.1f} - {np.max(wavelengths):.1f}nm")
    
    # Find 650nm wavelength (or closest)
    target_wl = 650.0
    tolerance = 5.0  # ±5nm tolerance
    
    # Find closest wavelength to 650nm
    wl_diffs = np.abs(wavelengths - target_wl)
    closest_idx = np.argmin(wl_diffs)
    closest_wl = wavelengths.iloc[closest_idx]
    closest_diff = wl_diffs.iloc[closest_idx]
    
    print(f"   Target: 650nm, Found: {closest_wl:.1f}nm (diff: {closest_diff:.1f}nm)")
    
    if closest_diff > tolerance:
        print(f"⚠️ No wavelength found near 650nm (closest: {closest_wl:.1f}nm, diff: {closest_diff:.1f}nm)")
        print(f"   Using closest available wavelength for normalization")
    
    reference_intensity = intensities.iloc[closest_idx]
    print(f"   Reference intensity at {closest_wl:.1f}nm: {reference_intensity:.2f}")
    
    if reference_intensity <= 0:
        print(f"❌ Invalid reference intensity at {closest_wl:.1f}nm: {reference_intensity}")
        return pd.DataFrame({0: wavelengths, 1: intensities}), {'method': 'Failed'}
    
    # Step 1: Normalize 650nm to 50000
    scale_factor_50k = 50000.0 / reference_intensity
    normalized_50k = intensities * scale_factor_50k
    
    print(f"   Step 1: Scale factor to 50000: {scale_factor_50k:.4f}")
    print(f"   Step 1: New range: {np.min(normalized_50k):.2f} - {np.max(normalized_50k):.2f}")
    print(f"   Step 1: 650nm now at: {normalized_50k.iloc[closest_idx]:.2f} (should be ~50000)")
    
    # Step 2: Scale entire spectrum to 0-100
    max_intensity = np.max(normalized_50k)
    scale_factor_100 = 100.0 / max_intensity
    final_intensities = normalized_50k * scale_factor_100
    
    print(f"   Step 2: Max was {max_intensity:.2f}, scale factor to 100: {scale_factor_100:.6f}")
    print(f"   Step 2: Final range: {np.min(final_intensities):.2f} - {np.max(final_intensities):.2f}")
    print(f"   ✅ Halogen normalization complete!")
    
    # Add normalization metadata
    normalization_info = {
        'method': 'Halogen_650nm_50000_to_100',
        'reference_wavelength': closest_wl,
        'reference_original_intensity': reference_intensity,
        'scale_factor_50k': scale_factor_50k,
        'scale_factor_100': scale_factor_100
    }
    
    return pd.DataFrame({0: wavelengths, 1: final_intensities}), normalization_info

def normalize_laser_max_to_50k(wavelengths, intensities):
    """L/Laser: Normalize max to 50000, then scale to 0-100"""
    
    max_intensity = np.max(intensities)
    max_idx = np.argmax(intensities)
    max_wavelength = wavelengths.iloc[max_idx]
    
    if max_intensity <= 0:
        print(f"❌ Invalid max intensity: {max_intensity}")
        return pd.DataFrame({0: wavelengths, 1: intensities})
    
    # Step 1: Normalize max to 50000
    scale_factor_50k = 50000.0 / max_intensity
    normalized_50k = intensities * scale_factor_50k
    
    print(f"   Step 1: Normalized max at {max_wavelength:.1f}nm ({max_intensity:.2f}) to 50000")
    print(f"           Scale factor: {scale_factor_50k:.4f}")
    
    # Step 2: Scale to 0-100
    current_max = np.max(normalized_50k)  # Should be 50000
    scale_factor_100 = 100.0 / current_max
    final_intensities = normalized_50k * scale_factor_100
    
    print(f"   Step 2: Scaled to 0-100 range")
    print(f"   ✅ Laser normalization complete: {np.min(final_intensities):.2f} - {np.max(final_intensities):.2f}")
    
    # Add normalization metadata
    normalization_info = {
        'method': 'Laser_max_50000_to_100',
        'reference_wavelength': max_wavelength,
        'reference_original_intensity': max_intensity,
        'scale_factor_50k': scale_factor_50k,
        'scale_factor_100': scale_factor_100
    }
...
def normalize_uv_811nm_to_15k(wavelengths, intensities):
    """U/UV: Normalize 811nm to 15000, then scale to 0-100"""
    
    # Find 811nm wavelength (or closest)
    target_wl = 811.0
    tolerance = 2.0  # ±2nm tolerance for UV
    
    # Find closest wavelength to 811nm
    wl_diffs = np.abs(wavelengths - target_wl)
    closest_idx = np.argmin(wl_diffs)
    closest_wl = wavelengths.iloc[closest_idx]
    closest_diff = wl_diffs.iloc[closest_idx]
    ...
    if closest_diff > tolerance:
        print(f"⚠️ No wavelength found near 811nm (closest: {closest_wl:.1f}nm, diff: {closest_diff:.1f}nm)")
        print(f"   Using closest available wavelength for normalization")
    ...
    # Step 1: Normalize 811nm to 15000
    scale_factor_15k = 15000.0 / reference_intensity
    normalized_15k = intensities * scale_factor_15k
    
    print(f"   Step 1: Normalized {closest_wl:.1f}nm ({reference_intensity:.2f}) to 15000")
    print(f"           Scale factor: {scale_factor_15k:.4f}")
    
    # Step 2: Scale entire spectrum to 0-100
    max_intensity = np.max(normalized_15k)
    scale_factor_100 = 100.0 / max_intensity
    final_intensities = normalized_15k * scale_factor_100
    
    print(f"   Step 2: Scaled to 0-100 range (max was {max_intensity:.2f})")
    print(f"   ✅ UV normalization complete: {np.min(final_intensities):.2f} - {np.max(final_intensities):.2f}")
    ...
    normalization_info = {
        ...
        'reference_wavelength': closest_wl,
        'reference_original_intensity': reference_intensity,
        'scale_factor_15k': scale_factor_15k,
        'scale_factor_100': scale_factor_100
    }
    
    return pd.DataFrame({0: wavelengths, 1: final_intensities}), normalization_info

def normalize_default_max_to_50k(wavelengths, intensities):
    """Default: Normalize max to 50000, then scale to 0-100 (same as Laser)"""
    print(f"   Using default max normalization (same as Laser)")
    return normalize_laser_max_to_50k(wavelengths, intensities)

def apply_normalization_after_baseline():
    """Apply light-source-specific normalization after baseline is established"""
    global spectrum_df, ax, baseline_data, filename
    
    if baseline_data is None:
        print("⚠️ No baseline established - cannot normalize properly")
        return False
    
    print(f"\n🔧 STARTING NORMALIZATION PROCESS:")
    print(f"   Current spectrum range: {np.min(spectrum_df.iloc[:, 1]):.2f} - {np.max(spectrum_df.iloc[:, 1]):.2f}")
    
    # Detect light source from filename
    light_code, light_name = detect_light_source_from_filename(filename)
    
    if light_code is None:
        print("⚠️ Could not detect light source - using default normalization")
        light_code = 'L'  # Default to Laser-style normalization
        light_name = 'Default'
    
    print(f"\n🔧 APPLYING LIGHT-SOURCE-SPECIFIC NORMALIZATION:")
    print(f"   Detected: {light_name} ({light_code})")
    print(f"   Pre-normalization spectrum_df shape: {spectrum_df.shape}")
    print(f"   Pre-normalization intensity range: {np.min(spectrum_df.iloc[:, 1]):.2f} - {np.max(spectrum_df.iloc[:, 1]):.2f}")
    
    # Apply the appropriate normalization
    result = normalize_spectrum_by_light_source(spectrum_df, light_code, filename)
    
    # Handle return value (may include normalization_info)
    if isinstance(result, tuple):
        spectrum_df, normalization_info = result
        print(f"   Normalization returned tuple: spectrum_df updated")
    else:
        spectrum_df = result
        normalization_info = {'method': f'{light_name}_normalization'}
        print(f"   Normalization returned dataframe: spectrum_df updated")
    
    print(f"   Post-normalization spectrum_df shape: {spectrum_df.shape}")
    print(f"   Post-normalization intensity range: {np.min(spectrum_df.iloc[:, 1]):.2f} - {np.max(spectrum_df.iloc[:, 1]):.2f}")
    
    # Recalculate baseline stats with normalized data
    start_wl = baseline_data['wavelength_start']
    end_wl = baseline_data['wavelength_end']
    
    print(f"   Recalculating baseline stats for region {start_wl:.1f}-{end_wl:.1f}nm...")
    
    updated_baseline = calculate_baseline_stats(spectrum_df, start_wl, end_wl)
    if updated_baseline:
        baseline_data.update(updated_baseline)
        baseline_data['normalization_applied'] = True
        baseline_data['light_source'] = light_name
        baseline_data['light_code'] = light_code
        baseline_data['normalization_method'] = normalization_info.get('method', f'{light_name}_normalization')
        
        # Add detailed normalization info
        if 'reference_wavelength' in normalization_info:
            baseline_data['reference_wavelength'] = normalization_info['reference_wavelength']
        if 'reference_original_intensity' in normalization_info:
            baseline_data['reference_original_intensity'] = normalization_info['reference_original_intensity']
        
        print(f"✅ Baseline recalculated after {light_name} normalization:")
        print(f"   New SNR: {baseline_data['snr']:.1f}")
        print(f"   New Avg Intensity: {baseline_data['avg_intensity']:.2f}")
    else:
        print(f"❌ Failed to recalculate baseline stats")
...
def update_spectrum_plot():
    ...
    global ax, spectrum_df, baseline_data
    
    if ax is None or spectrum_df is None:
        return
    
    # Clear previous spectrum line (keep markers)
    for line in ax.lines[:]:  # Only remove lines, not scatter plots
        line.remove()
    
    # Plot updated spectrum
    wavelengths = spectrum_df.iloc[:, 0]
    intensities = spectrum_df.iloc[:, 1]
    ax.plot(wavelengths, intensities, 'k-', linewidth=0.8, label='Spectrum')
    
    # FORCE Y-AXIS RESCALE to show normalized data properly
    ax.relim()  # Recalculate limits
    ax.autoscale_view()  # Rescale view
    
    # Set reasonable y-limits for normalized data (0-100 scale)
    if baseline_data and baseline_data.get('normalization_applied'):
        max_intensity = np.max(intensities)
        min_intensity = np.min(intensities)
        margin = (max_intensity - min_intensity) * 0.05  # 5% margin
        ax.set_ylim(min_intensity - margin, max_intensity + margin)
        
        print(f"🔄 Plot updated: Y-range {min_intensity:.2f} to {max_intensity:.2f}")
    
    # Update title to show normalization and light source status
    if baseline_data and baseline_data.get('normalization_applied'):
        light_source = baseline_data.get('light_source', 'Unknown')
        method = baseline_data.get('normalization_method', 'Applied')
        ax.set_title(f"Enhanced Structural Marker - {filename} [{light_source} NORMALIZED]")
    else:
        ax.set_title(f"Enhanced Structural Marker - {filename}")
    
    ax.legend()
    plt.draw()
    plt.show()  # Force refresh
...
def onclick(event):
    ...
    global current_type, spectrum_df, clicks, ax
    ...
    if current_type == 'Baseline Region':
        ...
        if len(clicks) == 2:
            ...
            if baseline_stats:
                ...
                # APPLY LIGHT-SOURCE-SPECIFIC NORMALIZATION AFTER BASELINE IS ESTABLISHED
                print(f"\n🔧 APPLYING LIGHT-SOURCE-SPECIFIC NORMALIZATION...")
                if apply_normalization_after_baseline():
                    light_source = baseline_data.get('light_source', 'Unknown')
                    method = baseline_data.get('normalization_method', 'Applied')
                    print(f"✅ {light_source} normalization complete - spectrum ready for analysis")
                else:
                    print(f"⚠️ Normalization failed - using raw data")
                
                # Add baseline entry to features (with updated normalized stats and light source info)
                entry = {
                    'Feature': 'Baseline Region',
                    'File': filename,
                    'Start': start_wl,
                    'End': end_wl,
                    'Avg_Intensity': baseline_data['avg_intensity'],
                    'Std_Deviation': baseline_data['std_deviation'],
                    'SNR': baseline_data['snr'],
                    'Slope': baseline_data['slope'],
                    'R_Squared': baseline_data['r_squared'],
                    'Data_Points': baseline_data['data_points'],
                    'Normalization_Applied': baseline_data.get('normalization_applied', False),
                    'Light_Source': baseline_data.get('light_source', 'Unknown'),
                    'Light_Code': baseline_data.get('light_code', 'Unknown'),
                    'Normalization_Method': baseline_data.get('normalization_method', 'None'),
                    'Reference_Wavelength': baseline_data.get('reference_wavelength', 'N/A'),
                    'Reference_Original_Intensity': baseline_data.get('reference_original_intensity', 'N/A')
                }
...
def undo_callback(event):
    ...
    if features:
        ...
        # If we removed baseline, clear baseline data and reset normalization
        if feature_type == 'Baseline Region':
            baseline_data = None
            print("⚠️ Baseline removed - raw intensities will be used")
            print("⚠️ Normalization reset - please re-establish baseline")
...
def onkey(event):
    ...
    if event.key == '1':
        ...
        force_plot_refresh(event)
    elif event.key == 'c':
        toggle_click_mode(event)
...
def save_callback(event):
    ...
    if features:
        ...
        try:
            ...
            if baseline_data:
                print(f"   Light source: {baseline_data.get('light_source', 'Unknown')} ({baseline_data.get('light_code', '?')})")
                print(f"   SNR: {baseline_data['snr']:.1f}")
                if baseline_data.get('normalization_applied'):
                    method = baseline_data.get('normalization_method', 'Applied')
                    ref_wl = baseline_data.get('reference_wavelength', 'N/A')
                    print(f"   Normalization: {method}")
                    if ref_wl != 'N/A':
                        print(f"   Reference wavelength: {ref_wl}nm")
                else:
            ...
            print(f"   Data includes: Wavelengths, Light-Source-Normalized Intensities, Slopes, Baseline Corrections")
...
def reset_globals():
    ...
    print("   Normalization state cleared")
...
def create_enhanced_ui(fig, ax):
    ...
    # Click Mode Toggle - NEW BUTTON TO FIX MARKER PLACEMENT
    ax_click_mode = fig.add_axes([button_x, 0.20, button_width, button_height])
    btn_click_mode = Button(ax_click_mode, 'Click Mode\n(Key: C)', color='lightcyan', hovercolor='cyan')
    btn_click_mode.on_clicked(toggle_click_mode)
    buttons.append(btn_click_mode)
    
    # Refresh Plot Button (for debugging normalization issues)
    ax_refresh = fig.add_axes([button_x, 0.14, button_width, button_height])
    btn_refresh = Button(ax_refresh, 'Refresh Plot\n(Key: R)', color='lightgray', hovercolor='gray')
    btn_refresh.on_clicked(force_plot_refresh)
    buttons.append(btn_refresh)
    
    return buttons

def toggle_click_mode(event):
    """Toggle between click position and curve interpolation for marker placement"""
    onclick.use_click_position = not onclick.use_click_position
    
    mode = "CLICK POSITION" if onclick.use_click_position else "CURVE INTERPOLATION"
    print(f"🎯 MARKER PLACEMENT MODE: {mode}")
    
    if onclick.use_click_position:
        print("   ✅ Markers will appear exactly where you click")
        print("   💡 Good for precise peak marking and manual intensity setting")
    else:
        print("   ✅ Markers will snap to the spectrum curve") 
        print("   💡 Good for accurate curve-based intensity measurement")

def force_plot_refresh(event):
    """Force refresh the plot - useful for debugging normalization issues"""
    global spectrum_df, ax, baseline_data
    
    print("🔄 FORCING PLOT REFRESH...")
    if spectrum_df is not None:
        print(f"   Current data range: {np.min(spectrum_df.iloc[:, 1]):.2f} - {np.max(spectrum_df.iloc[:, 1]):.2f}")
        update_spectrum_plot()
        print("✅ Plot refresh complete")
    else:
        print("❌ No spectrum data to refresh")
...
def run_enhanced_marker():
    ...
    # Detect light source from filename early
    light_code, light_name = detect_light_source_from_filename(filename)
    if light_code:
        print(f"🔍 Detected light source: {light_name} ({light_code})")
        if light_code == 'B':
            print(f"   Will normalize 650nm → 50000 → 0-100")
        elif light_code == 'L':
            print(f"   Will normalize max → 50000 → 0-100")
        elif light_code == 'U':
            print(f"   Will normalize 811nm → 15000 → 0-100")
    else:
        print(f"⚠️ Could not detect light source - will use default normalization")
    ...
    try:
        ...
        print("   - AUTO-NORMALIZATION by light source:")
        print("     • B/Halogen: 650nm→50000→0-100")
        print("     • L/Laser: max→50000→0-100")  
        print("     • U/UV: 811nm→15000→0-100")
        print("\n🎯 STEP 2: Mark spectral features normally")
        print("   - Tool now captures intensity + slope data")
        print("   - Uses normalized intensities for all measurements")
        print("   - Baseline correction applied automatically")
        print("\n🎯 MARKER PLACEMENT (FIXED):")
        print("   - If markers appear below clicks, press 'C' key")
        print("   - Click Mode: markers at exact click position")
        print("   - Curve Mode: markers snap to spectrum curve")
        print("\n📊 Enhanced data captured:")
        print("   ✅ Wavelength positions")
        print("   ✅ Normalized intensities (light-source specific)")
        print("   ✅ Local slope at each point")
        print("   ✅ Baseline statistics and SNR")
        print("   ✅ Light source detection and metadata")
        print("\n⌨️ KEYBOARD SHORTCUTS:")
        print("   1-7: Select feature types")
        print("   P: Toggle persistent mode")
        print("   C: Toggle click mode (fixes marker placement)")
        print("   M: Toggle magnify mode")
        print("   U: Undo last action")
        print("   S: Save data")
        print("   R: Refresh plot (debug normalization)")
        print("\n💡 MARKER PLACEMENT TIPS:")
        print("   • If markers appear below your clicks, press 'C' key")
        print("   • Click Mode = markers at exact click position")
        print("   • Curve Mode = markers snap to spectrum curve")
        print("   • Good for precise peak marking vs. curve accuracy")
